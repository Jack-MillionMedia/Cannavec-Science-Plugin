...
| `affinity --ic50 I --ligand L --kd K [--unit nM] [--mode] [--json]` | **Cheng–Prusoff IC50 → Ki** (spec 024): converts an assay-dependent IC50 into an assay-independent Ki + pKi so receptor affinities from different papers compare. Radioligand or enzyme mode; M / mM / µM / nM / pM units. In-vitro binding pharmacology only. |
