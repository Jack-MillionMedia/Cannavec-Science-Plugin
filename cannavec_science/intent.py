...
#
# Keyword-search backends (PubMed esearch, OpenAlex) mis-parse conversational
# phrasing: a typed question like "What is the molecular difference between THC
# and CBD?" is mapped by PubMed to `the, is[Author] AND ("molecular"...)` — it
# reads the short word "is" as an [Author] field, AND-joins every content term,
# and returns ZERO hits even though thousands of relevant papers exist.
# Stripping the interrogative scaffolding leaves the content terms the backend
# CAN map. Synthesis also reuses this to skip leading function words when it
# falls back to a title's first word for a cluster key.
#
# Conservative by design — ONLY closed-class function / interrogative words,
# never domain terms. A query already in keyword form ("THC CBD molecular
...
def distill_query(query: str) -> str:
    """Strip natural-language scaffolding so a question becomes search-mappable.
